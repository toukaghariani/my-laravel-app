...
<div class="pt-24 pb-16 px-4 md:px-12 max-w-screen-2xl mx-auto">
    ...
    <div class="text-center mb-12">
        ...
        <p class="text-gray-400 text-lg max-w-xl mx-auto">Unlock premium content, ad-free streaming, and unlimited access to our full library.</p>
    </div>
    ...
    @if($plans->count())
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-5xl mx-auto">
            @foreach($plans as $index => $plan)
                <div class="relative surface-card p-8 flex flex-col items-center text-center transition-all duration-300 hover:border-brand/50 hover:shadow-lg hover:shadow-brand/5 {{ $index === 1 ? 'ring-2 ring-brand' : '' }}">
                    {{-- Popular badge --}}
                    @if($index === 1)
                        <div class="absolute -top-3 left-1/2 -translate-x-1/2">
                            <span class="bg-brand text-white text-xs font-bold px-4 py-1 rounded-full uppercase tracking-wider">Most Popular</span>
                        </div>
                    @endif
    ...
    <div class="max-w-2xl mx-auto mt-16 text-center">
        ...
        <p class="text-gray-500">All subscriptions include full access to our premium library. Cancel anytime. Payments are processed securely through Flouci.</p>
    </div>
</div>
@endsection
